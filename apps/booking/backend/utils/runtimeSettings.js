...
const DEFAULT_RUNTIME_SETTINGS = [
  ...
  {
    key: 'family.invitation.acceptOtpRequired.enabled',
    type: 'boolean',
    group: 'Family Booking',
    label: 'Require OTP to accept family invitations',
    description: 'When enabled, an existing client must enter a one-time SMS code before accepting a family invitation. Decline and Report remain available without OTP. Disabled by default.',
    defaultValue: envBool('FAMILY_INVITATION_ACCEPT_OTP_REQUIRED', false),
  },
  {
    key: 'family.invitation.baseUrl',
    type: 'string',
    group: 'Family Booking',
    label: 'Family invitation page base URL',
    description: 'Base booking-app URL used to build secure family invitation links. Production normally uses https://rakiesalon.com/booking.',
    defaultValue: envString('FAMILY_INVITATION_BASE_URL', process.env.FRONTEND_BASE_URL || 'https://rakiesalon.com/booking'),
    isAdvanced: true,
  },
  {
    key: 'family.invitation.declineCooldownHours',
    type: 'number',
    group: 'Family Booking',
    label: 'Declined family invitation cooldown hours',
    description: 'How long a declined family invitation must wait before the same pair can be invited again. Default is 24 hours. Decimal values are allowed for testing (0.001 hour is about 4 seconds); 0 disables the automatic cooldown. Authorized staff can clear the cooldown early from the client profile.',
    defaultValue: envNumber('FAMILY_INVITATION_DECLINE_COOLDOWN_HOURS', 24),
  },
  {
    key: 'family.invitation.linkExpiryHours',
    type: 'number',
    group: 'Family Booking',
    label: 'Family invitation link expiry hours',
    description: 'How many hours a secure family invitation link remains usable. Default is 48 hours.',
    defaultValue: envNumber('FAMILY_INVITATION_LINK_EXPIRY_HOURS', 48),
    isAdvanced: true,
  },
  {
  ...
  {
    key: 'booking.pendingAdminSms.enabled',
    type: 'boolean',
    group: 'Booking Controls',
    label: 'Text admins for pending online bookings',
    description: 'When enabled, each new customer-created pending booking sends an SMS review link to the authorized admin phone numbers below.',
    defaultValue: envBool('PENDING_BOOKING_ADMIN_SMS_ENABLED', true),
  },
  {
    key: 'booking.pendingAdminSms.recipients',
    type: 'string',
    group: 'Booking Controls',
    label: 'Pending booking admin SMS phones',
    description: 'Authorized admin phone numbers that receive pending-booking approval links. Separate multiple US numbers with commas.',
    defaultValue: envString('PENDING_BOOKING_ADMIN_PHONES', ''),
  },
  {
    key: 'booking.pendingAdminSms.tokenHours',
    type: 'number',
    group: 'Booking Controls',
    label: 'Pending booking approval link hours',
    description: 'How long an SMS approval link remains valid. The link also stops working as soon as the appointment is no longer pending.',
    defaultValue: envNumber('PENDING_BOOKING_APPROVAL_HOURS', 24),
    isAdvanced: true,
  },
  {
